    ...
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Validés", val: comptesAvecCandidatureApprouvee.filter(c => c.statut_kyc === "valide").length, couleur: "text-emerald-600" },
          { label: "Rejetés", val: comptesAvecCandidatureApprouvee.filter(c => c.statut_kyc === "rejete").length, couleur: "text-red-600" },
        ].map(({ label, val, couleur }) => (
          <div key={label} className="bg-white rounded-xl border border-slate-200 p-4 text-center">
            <p className={`text-2xl font-bold ${couleur}`}>{val}</p>
            <p className="text-xs text-slate-500 mt-1">{label}</p>
